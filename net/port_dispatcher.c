...
void port_dispatcher_task(port_dispr_t *dispr)
{
...

    {
        ...
        while (item) {
            ...
            if (node) {
                ...
                // or manual set by user?
                if (pkt->src_port == CDNET_DEF_PORT && pkt->dst_port <= 63 &&
                        !pkt->is_multi_net && !pkt->is_seq
                        && !pkt->multicast_id)
                    pkt->level = CDNET_L0;
                else
                    pkt->level = CDNET_L1;
    ...
    node = list_get(&dispr->V_ser_head);
    if (node) {
        cdnet_packet_t *pkt = container_of(node, cdnet_packet_t, node);
